"""Shared Stage 01 importer for the isolated LEAP/Wuji2 A/B reproduction.

Run one of the thin branch ``01_import.py`` entries instead of running this
file directly.  All branches share the scene and evaluator physics.
"""
...
ALLOWED_BRANCHES = {"wuji2_unified_weighted", "wuji2_leap_root_drive"}
...
async def import_branch() -> None:
    ...
    def reset_actors() -> None:
        ...
        if BRANCH == "wuji2_unified_weighted":
            hand.set_world_pose(
                position=root_poses[0, :3, 3],
                orientation=matrix_to_quaternion_wxyz(root_poses[0, :3, :3]),
            )
    ...
    context = {
        ...
        "root_control_policy": (
            ...
            if BRANCH == "wuji2_leap_root_drive"
            else "legacy_direct_r_wrist_pose_servo"
        ),
