"""Shared Stage 02 executor for the isolated LEAP/Wuji2 A/B reproduction."""
...
def on_physics_step(_step_size: float) -> None:
    ...
    if BRANCH == "wuji2_unified_weighted":
        start_pose = root_poses[segment - 1]
        end_pose = root_poses[segment]
        position = start_pose[:3, 3] + (end_pose[:3, 3] - start_pose[:3, 3]) * alpha
        orientation = slerp(
            rotation_to_quaternion_wxyz(start_pose[:3, :3]),
            rotation_to_quaternion_wxyz(end_pose[:3, :3]),
            alpha,
        )
        # Legacy comparison branch only.  The LEAP-style branch never enters
        # this block: its six root DOFs are part of ``joint_target`` and use
        # the same ArticulationAction path as the official LEAP evaluator.
        hand.set_world_pose(position=position, orientation=orientation)
        hand.set_linear_velocity(np.zeros(3, dtype=np.float32))
        hand.set_angular_velocity(np.zeros(3, dtype=np.float32))
